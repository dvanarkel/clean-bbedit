...
where
	...
	# world = writeMessage (generateMessage $ didSaveNotificationBodyFor $ testModulePathFor curDir "ok") io.stdIn world
	# (message, world) = readMessage io.stdOut world
	# world = shutdownLanguageServer handle world
	= (message =.= generateMessage (expectedDidSaveNotificationResponseBodyFor $ testModulePathFor curDir "ok"), world)

	testModulePathFor :: !FilePath !FilePath -> FilePath
	testModulePathFor dir name = dir </> "testPrograms" </> name <.> "icl"
...
didSaveNotificationBodyFor file =
	concat3
		...
		file
		"\"}}}"
expectedDidSaveNotificationResponseBodyFor file =
	concat3
